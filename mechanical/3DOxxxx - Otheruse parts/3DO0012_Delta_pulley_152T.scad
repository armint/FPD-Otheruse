...
module DeltaPulley(teeth) {
    ...
    difference() {
        translate([-sliceoff,0,0])rotate([0,0,-30])difference() {
            union() {
                pulleyTeeth(teeth);
                // limit switch interruptor
                linear_extrude(drive_wheel_height+18)rotate(-150)arc(r=41.5, w=1, angle=150, $fa=1);
            }
            // axis hole
            translate([0,0,-1])cylinder(d=m8_dia, h=drive_wheel_height+2);
            // screw holes
            for (a=[0:90:359]) {
                rotate([0,0,a-30])translate([10,0,-1])cylinder(d=m3_dia, h=drive_wheel_height+2);
            }
            // tie wrap slot
            rotate([0,0,90]) {
                translate([25,-5,drive_wheel_height-2])cube([3,10,3]);
                translate([25,-5,-1])cube([3,10,3]);
            }
        }
        // Slice pulley
        translate([0,-wheel_radius,-1])cube([2*wheel_radius,2*wheel_radius,drive_wheel_height+2]);
        // belt slot
        translate([-5,-15,0])beltSlot(drive_wheel_height);
    }
    // belt slot print support
    color([1,0,0,1])for (i=[-5:10:5]) {
        rotate([0,0,0])translate([-5, -15+i, 2.5-print_layer_height])cylinder(d=m3_nut_dia, h=print_layer_height);
    }
    ...
    difference() {
        union() {
            difference() {
                ...
                translate([0,0,drive_wheel_height/2])rotate_extrude()translate([wheel_radius-1,0]) {
                trapezoid(top=drive_wheel_height-3,base=drive_wheel_height-3-2*flange,h=3.5);
                }
            }
            pulley(12, teeth);
        }
...
// Based on pulleys by:
// https://www.thingiverse.com/thing:16627 by droftarts
...
module pulley(profile, teeth) {
    // profile: 1=MXL 2=40DP 3=XL 4=H 5=T2.5 6=T5 7=T10 8=AT5 9=HTD_3mm 10=HTD_5mm 11=HTD_8mm 12=GT2_2mm 13=GT2_3mm 14=GT2_5mm
    // The following set the pulley diameter for a given number of teeth

    MXL_pulley_dia = tooth_spacing (teeth,2.032,0.254);
    40DP_pulley_dia = tooth_spacing (teeth,2.07264,0.1778);
    XL_pulley_dia = tooth_spacing (teeth,5.08,0.254);
    H_pulley_dia = tooth_spacing (teeth,9.525,0.381);
    T2_5_pulley_dia = tooth_spaceing_curvefit (teeth,0.7467,0.796,1.026);
    T5_pulley_dia = tooth_spaceing_curvefit (teeth,0.6523,1.591,1.064);
    T10_pulley_dia = tooth_spacing (teeth,10,0.93);
    AT5_pulley_dia = tooth_spaceing_curvefit (teeth,0.6523,1.591,1.064);
    HTD_3mm_pulley_dia = tooth_spacing (teeth,3,0.381);
    HTD_5mm_pulley_dia = tooth_spacing (teeth,5,0.5715);
    HTD_8mm_pulley_dia = tooth_spacing (teeth,8,0.6858);
    GT2_2mm_pulley_dia = tooth_spacing (teeth,2,0.254);
    GT2_3mm_pulley_dia = tooth_spacing (teeth,3,0.381);
    GT2_5mm_pulley_dia = tooth_spacing (teeth,5,0.5715);

    // The following calls the pulley creation part, and passes the pulley diameter and tooth width to that module

    if ( profile == 1 ) { pulley_base ( "MXL", profile, teeth , MXL_pulley_dia , 0.508 , 1.321 ); }
    if ( profile == 2 ) { pulley_base ( "40 D.P.", profile, teeth , 40DP_pulley_dia , 0.457 , 1.226 ); }
    if ( profile == 3 ) { pulley_base ( "XL", profile, teeth , XL_pulley_dia , 1.27, 3.051 ); }
    if ( profile == 4 ) { pulley_base ( "H", profile, teeth , H_pulley_dia ,1.905 , 5.359 ); }
    if ( profile == 5 ) { pulley_base ( "T2.5", profile, teeth , T2_5_pulley_dia , 0.7 , 1.678 ); }
    if ( profile == 6 ) { pulley_base ( "T5", profile, teeth , T5_pulley_dia , 1.19 , 3.264 ); }
    if ( profile == 7 ) { pulley_base ( "T10", profile, teeth , T10_pulley_dia , 2.5 , 6.13 ); }
    if ( profile == 8 ) { pulley_base ( "AT5", profile, teeth , AT5_pulley_dia , 1.19 , 4.268 ); }
    if ( profile == 9 ) { pulley_base ( "HTD 3mm", profile, teeth , HTD_3mm_pulley_dia , 1.289 , 2.27 ); }
    if ( profile == 10 ) { pulley_base ( "HTD 5mm", profile, teeth , HTD_5mm_pulley_dia , 2.199 , 3.781 ); }
    if ( profile == 11 ) { pulley_base ( "HTD 8mm", profile, teeth , HTD_8mm_pulley_dia , 3.607 , 6.603 ); }
    if ( profile == 12 ) { pulley_base ( "GT2 2mm", profile, teeth , GT2_2mm_pulley_dia , 0.764 , 1.494 ); }
    if ( profile == 13 ) { pulley_base ( "GT2 3mm", profile, teeth , GT2_3mm_pulley_dia , 1.169 , 2.31 ); }
    if ( profile == 14 ) { pulley_base ( "GT2 5mm", profile, teeth , GT2_5mm_pulley_dia , 1.969 , 3.952 ); }
}
// Main Module

module pulley_base( belt_type, profile , teeth, pulley_OD , tooth_depth , tooth_width ) {
...
function pulley_dia (profile, teeth)  =   
    profile == 1 ? tooth_spacing (teeth,2.032,0.254) :
    profile == 2 ? tooth_spacing (teeth,2.07264,0.1778) :
    profile == 3 ? tooth_spacing (teeth,5.08,0.254) :
    profile == 4 ? tooth_spacing (teeth,9.525,0.381) :
    profile == 5 ? tooth_spaceing_curvefit (teeth,0.7467,0.796,1.026) :
    profile == 6 ? tooth_spaceing_curvefit (teeth,0.6523,1.591,1.064) :
    profile == 7 ? tooth_spacing (teeth,10,0.93) :
    profile == 8 ? tooth_spaceing_curvefit (teeth,0.6523,1.591,1.064) :
    profile == 9 ? tooth_spacing (teeth,3,0.381) :
    profile == 10 ? tooth_spacing (teeth,5,0.5715) :
    profile == 11 ? tooth_spacing (teeth,8,0.6858) :
    profile == 12 ? tooth_spacing (teeth,2,0.254) :
    profile == 13 ? tooth_spacing (teeth,3,0.381) :
    profile == 14 ? tooth_spacing (teeth,5,0.5715) : 0;
